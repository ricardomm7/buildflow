...
int removeMachine(Machine **head, int id) {
    ...
    while (current) {
        if (current->id == id) {
            // Check if any operation is in progress (OP state)
            int hasActiveOperations = 0;
            for (int i = 0; i < current->operationCount; i++) {
                if (strcmp(current->operations[i].state, "OP") == 0) {
                    hasActiveOperations = 1;
                    break;
                }
            }
            
            // If there are active operations, prevent removal
            if (hasActiveOperations) {
                printf("Cannot remove machine with active operations (status: OP).\n");
                return 0;
            }
            
            // If no active operations, proceed with removal
            if (prev) 
                prev->next = current->next;
            else 
                *head = current->next;
            
            // Free memory for operations and machine
